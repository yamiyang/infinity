...
```bash
npm install wc-components
# or
pnpm add wc-components
```
...
```html
<script src="https://unpkg.com/wc-components/dist/wc-components.iife.js"></script>
...
```js
import { defineAll, initWCHost, configFromPreset } from "wc-components";
...
import { initWCHost, configFromPreset } from "wc-components";
...
```js
import { WCBase } from "wc-components";
